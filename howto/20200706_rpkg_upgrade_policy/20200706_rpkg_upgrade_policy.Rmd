...
In case, we want to run different images on the same host, we have to specify for each image a different local R library search path. This can be done by including the name of the container image into the local R library search path. This must already be setup in the container post-installation step. The definition of the R library search path is included in the '.Rprofile' startup file. The '.Rprofile' file can automatically be generated using the script 'update_rprofile.sh'. 
